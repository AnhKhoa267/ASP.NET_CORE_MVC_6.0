...
namespace ShoppingCart.DataAccess.Repositories
{
    public class Repository<T> : IRepository<T> where T : class
    {
        ...
        public T GetT(Expression<Func<T, bool>> predicate, string? includeProptise=null)
        {
            IQueryable<T> query = _dbSet;
            query = query.Where(predicate);
            if (includeProptise != null)
            {
                foreach (var item in includeProptise.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries))
                {
                    query = query.Include(item);
                }
            }
